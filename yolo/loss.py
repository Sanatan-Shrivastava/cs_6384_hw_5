...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    loss_w = weight_coord * torch.sum(box_mask * torch.pow(gt_box[:, 2:5*num_boxes:5] - output[:, 2:5*num_boxes:5], 2.0))
    loss_h = weight_coord * torch.sum(box_mask * torch.pow(gt_box[:, 3:5*num_boxes:5] - output[:, 3:5*num_boxes:5], 2.0))
    loss_noobj = weight_noobj * torch.sum((1 - box_mask) * torch.pow(box_confidence - output[:, 4:5*num_boxes:5], 2.0))
    loss_cls = torch.sum(box_mask * torch.pow(gt_mask - output[:, 5*num_boxes:5*num_boxes+num_classes], 2.0))
